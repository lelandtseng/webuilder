...
var job = new Model('jobs','/home/lelandtseng/tmp');
...
function yz(req,res,next){
    if(req.session.loginuser){
            if(req.params.id){
            job.find({_id:new ObjectID(req.params.id)}, {},function(ps){
...
app.get('/new', yz2, form, function(req, res){
    res.render('new.html',{layout:false,errmsg:[],job:{},validatnum:req.validatnum});
});

app.get('/:id', function(req, res){
    job.get(new ObjectID(req.params.id), function(data){
...
app.post('/create',yz,form,function(req, res){
    if (req.errmsg) {
       ...
       job.save(req.formdata, function(){
            res.send("success");
       });  
    }
});
...
app.post("/:id/update", yz2, form, function(req, res){
    if (req.errmsg) {
        ...
        job.update(new ObjectID(req.params.id), req.formdata, function(){
            res.send("success");
        });
    }
})
...
app.del('/:id',yz,function(req,res){
    job.remove(new ObjectID(req.params.id),function(){
        res.send("success");
    });
});
